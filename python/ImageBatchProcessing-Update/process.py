...
class MyFrame(wx.Frame):
    def __init__(self, parent, id):
        wx.Frame.__init__(self, parent, id, "图片批处理", size=(590, 580))
        self.rb = "无"
        self.size = "256,256"
        self.SNR = 1
        self.num = 0
    ...
    def InitUI(self):
        ...
        pic = wx.Image('google.png', wx.BITMAP_TYPE_PNG)
        pic = pic.Scale(50, 50).ConvertToBitmap()
        bt_pic = wx.BitmapButton(self.FunPanel, -1, pic)
        bt_pic.Bind(wx.EVT_BUTTON, self.OnWeb)
        vbox.Add(bt_pic, 0, wx.ALIGN_CENTER_HORIZONTAL, 20)
        ...
        change = wx.BoxSizer(wx.HORIZONTAL)
        bt_next = wx.Button(self.DisPanel, -1, label="下一幅")
        bt_next.SetBackgroundColour("#00ff00")
        bt_next.SetForegroundColour("#ffffff")
        bt_next.Bind(wx.EVT_BUTTON, self.OnNext)
        change.Add(bt_next, 0, wx.EXPAND, 20)

        bt_prio = wx.Button(self.DisPanel, -1, label="上一幅")
        bt_prio.SetBackgroundColour("#00ffff")
        bt_prio.Bind(wx.EVT_BUTTON, self.OnPrio)
        change.Add(bt_prio, 0, wx.EXPAND, 20)
        dbox.Add(change, 0, wx.ALIGN_CENTER_HORIZONTAL)
        ...
        self.log.AppendText("\n\n下面显示处理日志：\n\n")
        self.log.AppendText("注意：文件目录不能有中文！\n")
    ...
    def OnclickStart(self, event):
        self.log.AppendText("--**处理图片预览**--\n")
        if self.image_path is None:
            self.log.AppendText("错误！没有选择输入文件！！\n")
            self.log.AppendText("请先选择输入文件，然后浏览！\n")
        else:
            image_path = self.image_path
            image = cv2.imread(image_path[self.index], cv2.IMREAD_COLOR)
            ...
            input_name = os.path.basename(image_path[self.index])

            if self.rb == "无":
                self.log.AppendText("->浏览图片<-\n"
                                    "-->图片名称：%s\n"
                                    "-->图片大小：(%d,%d)\n\n" % (input_name, H, W))
    ...
    def OnNext(self, event):
        if self.image_path is None:
            self.log.AppendText("错误！没有选择输入文件！！\n")
            self.log.AppendText("请先选择输入文件，然后切换！\n")
        else:
            self.index = self.index + 1
            if self.index >= self.num:
                self.index = 0
            image = cv2.imread(self.image_path[self.index], cv2.IMREAD_COLOR)
            image = cv2.resize(image, (200, 200))
            image = cv2.cvtColor(np.uint8(image), cv2.COLOR_BGR2RGB)
            pic = wx.Bitmap.FromBuffer(200, 200, image)
            self.original_image.SetBitmap(pic)

    def OnPrio(self, event):
        if self.image_path is None:
            self.log.AppendText("错误！没有选择输入文件！！\n")
            self.log.AppendText("请先选择输入文件，然后切换！\n")
        else:
    ...
    def OnWeb(self, event):
        web.open("https://github.com/pwolp/PwolpCode/blob/master/python/ImageBatchProcessing/process.py")
    ...
    def AddFlag(self, image):
        ...
        w_flag, h_flag = flag.shape[:2]
        # 计算图案缩放比例
        scale = w_head / w_flag / 4
        # 缩放图案
        flag = cv2.resize(flag, (0, 0), fx=scale, fy=scale)
        # 获取缩放后新宽度
        w_flag, h_flag = flag.shape[:2]
        # 按3个通道合并图片
        for c in range(0, 3):
            image[w_head - w_flag:, h_head - h_flag:, c] = flag[:, :, c]
